...
the workflows folder.
 ...
 2. "I am somewhat familiar with Python, but what are all these colons and the "->" symbols in the function
    definitions?"  These are type annotations and they can be ignored.  Many code editors can read 
    them and use this information to provide supplemental information and check for coding 
    mistakes.  Below are some examples and how to read them:
    1. "X: T", the variable X is of type T.  (Note "X: T = None", X is of type T or is None, and will 
       default to None unless specified otherwise)
    2. "def func() -> T:", the function func returns something of type T
    3. "T1 | T2", "either type T1 or type T2"
    4. "T1[T2]", type T1 is parameterized by type T2, for example "list[int]" means "a list of integers". 
    5. "X: Literal[1, 2]", the variable X will be either 1 or 2 (cannot be 3, "dog", "47", or anything else)
 3. Why are workflows specified by *.py files and not a more human-readable scripting language?
 ...
 4. "Can I use this code to make the platform act like a normal liquid handler?" Yes, see the map_assay.py file in
